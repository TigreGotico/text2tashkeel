"""Experiment: gate with rawi-v2 (best WHERE) but take the mark from V3 value
head (best WHICH, DER* 2.92%). Can it beat the rawi-v2+rawi-int8 baseline (2.20%)?"""
...
def _init(_):
    ...
    _MODELS = {
        "rawi-v2+rawi-int8": _EnsembleBackend(["rawi-v2"], "rawi-int8", "any"),       # 2.20% baseline
        "rawi-v2+rawi-v3": _EnsembleBackend(["rawi-v2"], "rawi-v3", "any"),           # v2 where, v3 value
        "rawi-v2-int8+rawi-v3-int8": _EnsembleBackend(["rawi-v2-int8"], "rawi-v3-int8", "any"),
    }
...
def main():
    ...
    names = ["rawi-v2+rawi-int8", "rawi-v2+rawi-v3", "rawi-v2-int8+rawi-v3-int8"]
