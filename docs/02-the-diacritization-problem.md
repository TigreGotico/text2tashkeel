...
will systematically miss these. This single requirement is the reason the
architecture (next page) uses:

1. a **bi**directional network — to read context from both sides, and
2. an **attention** mechanism — to let each character pull in information from
   *any* position in the sentence, near or far.
